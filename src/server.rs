...
use rmcp::{handler::server::wrapper::Parameters, tool, tool_handler, tool_router, ServerHandler, model::*};
...
use crate::tools::items::{ItemGetArgs, ItemUpdateArgs, ItemDeleteArgs, handle_zotero_get_item, handle_zotero_update_item, handle_zotero_delete_item};
use crate::tools::annotations::{GetAnnotationsArgs, GetNotesArgs, AddNoteArgs, handle_zotero_get_annotations, handle_zotero_get_notes, handle_zotero_add_note};
use crate::tools::collections::{ListCollectionsArgs, ManageCollectionsArgs, handle_zotero_list_collections, handle_zotero_manage_collections};
use crate::tools::library::{GetRecentArgs, ListTagsArgs, DeduplicateArgs, FetchArgs, handle_zotero_get_recent, handle_zotero_list_tags, handle_zotero_deduplicate, handle_fetch};
...
impl ZoteroServer {
    ...
    // === Items (3 tools) ===
    #[tool(description = "Get item metadata, children, fulltext, and/or BibTeX sections for a Zotero item.")]
    ...
    #[tool(description = "Get all annotations for a specific item or across your entire Zotero library.")]
    async fn zotero_get_annotations(&self, Parameters(args): Parameters<GetAnnotationsArgs>) -> String {
    ...
    // === Collections (2 tools) ===
    #[tool(description = "List collections, search collections, or retrieve items in a collection.")]
    async fn zotero_list_collections(&self, Parameters(args): Parameters<ListCollectionsArgs>) -> String {
    ...
    async fn zotero_manage_collections(&self, Parameters(args): Parameters<ManageCollectionsArgs>) -> String {
    ...
    // === Add Paper (1 tool) ===
    #[tool(description = "Add a paper to Zotero by URL, DOI, arXiv ID, ISBN, or local file path. Automatically extracts metadata and downloads PDF via WebDAV.")]
    ...
    // === Rename (1 tool) ===
    #[tool(description = "Rename PDF/file attachments of a Zotero item using Zotero's naming template (default: 'firstCreator - year - title.ext').")]
